...
using PhotoFileFilter.Features.Review.Controls;
using PhotoFileFilter.Features.Review.Models;
using PhotoFileFilter.Features.Review.Services;
using PhotoFileFilter.Features.Review.ViewModels;
using PhotoFileFilter.Features.TxtFilter.ViewModels;
using PhotoFileFilter.Features.TxtFilter.Views;
using PhotoFileFilter.Shared.Services;

namespace PhotoFileFilter.Features.Review.Views;
...
public partial class ReviewWindow : Window
{
    ...
    private void ShowFilterScreen(string? textListPath = null)
    {
        ...
        if (_filterView.DataContext is MainViewModel filter && string.IsNullOrWhiteSpace(filter.SourceFolder) && Directory.Exists(_viewModel.Folder))
            filter.SourceFolder = _viewModel.Folder;
        if (_filterView.DataContext is MainViewModel targetFilter && !string.IsNullOrWhiteSpace(textListPath))
        {
        ...
        if (_filterView.DataContext is MainViewModel filterTheme) filterTheme.DarkMode = true;
    ...
    {
        var filterBusy = _filterView?.DataContext is MainViewModel { Busy: true };
